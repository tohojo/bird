...
struct babel_write_state {
  ...
  u8 next_hop_v4_seen;
  u8 next_hop_v6_seen;
  u8 def_ip6_prefix[16];	/* Previous IPv6 prefix in network order (for compression) */
  u8 def_ip6_pxlen;
};
...
static inline uint bytes_equal(u8 *b1, u8 *b2, uint maxlen)
{
  uint i;
  for(i = 0; *b1 == *b2 && i < maxlen; i++, b1++, b2++)
    ;
  return i;
}
		     ...
		     struct babel_write_state *state, uint max_len)
{
  ...
  if (ipa_is_ip4(addr) && !state->next_hop_v4_seen) {
    ...
    state->next_hop_v4_seen = 1;

    return len;
  }
  else if (ipa_is_ip6(addr) && ip6_nonzero(addr) && !state->next_hop_v6_seen)
  {
                  ...
                  struct babel_parse_state *state)
{
  ...
  case BABEL_AE_IP4:
    ...
    msg->next_hop = state->next_hop_v4;
  ...
  case BABEL_AE_IP6:
    ...
    msg->next_hop = state->next_hop_v6;
                   ...
                   struct babel_write_state *state, uint max_len)
{
  ...
  int l = babel_write_next_hop((void *)tlv, msg->next_hop, state, max_len - len0);
  if (l < 0)
    return 0;
  ...
  else
  {
    ...
    omit = bytes_equal(buf, state->def_ip6_prefix, state->def_ip6_pxlen/8);
    DBG("%d bytes common between %I and previous prefix\n", omit,
	net6_prefix(&msg->net));

    if (0 < omit)
    {
      omit = MIN(omit, tlv->plen/8);
      memcpy(tlv->addr, buf+omit, BYTES(tlv->plen) - omit);
    ...
    else
    {
      ...
      put_ip6_px(state->def_ip6_prefix, &msg->net);
